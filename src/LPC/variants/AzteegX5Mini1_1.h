...
const size_t Heaters = 2; //Azteeg X5 Mini (Bed + Hotend)
...
const size_t MaxAxes = 4;						// The maximum number of movement axes in the machine, usually just X, Y and Z, <= DRIVES
...
// DRIVES
//                                              X      Y      Z      E1
const Pin ENABLE_PINS[DRIVES] =             { P0_10, P0_19, P0_21, P0_4};
const Pin STEP_PINS[DRIVES] =               { P2_1,  P2_2,  P2_3,  P2_0};
const uint8_t STEP_PIN_PORT2_POS[DRIVES] =  { 1,     2,     3,     0}; //SD: Used for calculating bitmap for stepping drivers (this is position of the pins on the port)
const uint32_t STEP_DRIVER_MASK =           0xFFFFFFF0; //SD: mask of the step pins on Port 2 used for writing to step pins in parallel (binary 0's indicate which bits to set)
const Pin DIRECTION_PINS[DRIVES] =          { P0_11, P0_20, P0_22, P0_5};
...
const Pin END_STOP_PINS[DRIVES] = { P1_24, P1_26, P1_28, NoPin}; // E stop could be mapped to a spare pin if needed...


//Azteeg X5MiniV1.1 uses MCP4451-103

// Indices for motor current digipots (X,Y,Z,E)
const uint8_t POT_WIPES[4] = { 0, 1, 2, 3};
...
//                                            Bed     Hotend
const Pin TEMP_SENSE_PINS[Heaters] = HEATERS_(P0_23, P0_24, c, d, e, f, g, h);
...
const Pin HEAT_ON_PINS[Heaters] = HEATERS_(P2_7, P2_5, c, d, e, f, g, h); // bed, h0
...
const Pin SdCardDetectPins[NumSdCards] = { NoPin, P1_31 }; //2nd card  CS pin P1_31
const Pin SdWriteProtectPins[NumSdCards] = { NoPin, NoPin };
const Pin SdSpiCSPins[NumSdCards] = { P0_6, P1_23 };// Internal, external. Note:: ("slot" 0 in CORE is configured to be LCP SSP1 to match default RRF behaviour)
