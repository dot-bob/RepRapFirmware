...
const size_t Heaters = 3; //Smoothie (Bed + H1 + H2)
...
//                                    X      Y      Z     E0      E1
const Pin END_STOP_PINS[DRIVES] = { P1_25, P1_27, P1_29, NoPin, NoPin}; // E stop could be mapped to a spare endstop pin if needed...
...
const float digipotFactor = 113.33; //factor for converting current to digipot value
...
const Pin HEAT_ON_PINS[Heaters] = HEATERS_(P2_5, P2_7, P1_23, d, e, f, g, h); // bed, h0, h1
...
const Pin COOLING_FAN_PINS[NUM_FANS] = { P2_4, P2_6 }; //Note: P2_6 is not hardware PWMable!!
...
//default to supporting 2 card..... if need 0_16 then change CS no No pin

const size_t NumSdCards = 2; //
const Pin SdCardDetectPins[NumSdCards] = { NoPin, NoPin };//TODO: 2nd cd CS pin?
const Pin SdWriteProtectPins[NumSdCards] = { NoPin, NoPin };
const Pin SdSpiCSPins[NumSdCards] = { P0_6, P0_16 };// Internal, external. Note:: ("slot" 0 in CORE is configured to be LCP SSP1 to match default RRF behaviour)
